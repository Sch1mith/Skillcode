...
interface GameOverProps {
  ...
  onRestart: () => void;
  userId?: number;
  level: 'junior' | 'pleno' | 'senior'; // Adicionado para saber o nível
}

export function GameOver({ score, totalQuestions, onRestart, userId, level }: GameOverProps) {
  const percentage = (score / totalQuestions) * 100;

  React.useEffect(() => {
    if (userId && level) {
      let body: any = {};
      if (level === 'junior') body.pontuacao_junior = score;
      if (level === 'pleno') body.pontuacao_pleno = score;
      if (level === 'senior') body.pontuacao_senior = score;
      fetch(`/api/usuarios/${userId}/pontuacao`, {
        method: 'PUT',
